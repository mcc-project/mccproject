...
Value sendmessagefrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "\nSent an message using funds from an account to a mcc address.\n" +
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "\nExamples:\n"
                                        "\nSend \"hello mcc\" from the default account to the address.\n" +
            HelpExampleCli("sendmessagefrom", "\"\" \"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" \"hello mcc\"") +
            "\nSend \"hello mcc\" from the tabby account to the given address.\n" + HelpExampleCli("sendmessagefrom", "\"tabby\" \"hello mcc\"") +
            "\nAs a json rpc call\n" + HelpExampleRpc("sendmessagefrom", "\"tabby\", \"hello mcc\""));
    ...
    if (MESSAGE_FEE > nBalance)
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "Account has insufficient funds (0.001 MCC required)");
...
Value sendmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n" +
            HelpExampleCli("sendmessage", "\"hello mcc\""));
