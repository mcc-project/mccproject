...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MCC address (e.g. %1)").arg("MMcJVNK6LNTKBpZ5hscqciLjwBbxkCknzu"));
#endif
...
{
    // return if URI is not valid or is no MCC: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert mcc:// to mcc:
    //
    //    Cannot handle this later, because mcc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open mcc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MCC.lnk";
}
...
{
    return GetAutostartDir() / "mcc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a mcc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the mcc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add mcc app to startup item list
